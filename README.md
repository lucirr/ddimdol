...
=================================================================
  ...
  Portal API  ──publish──>  NATS JetStream
  (릴리스 발행)              ├─ RELEASES 스트림
...
========================================================================
                            │
           ┌────────────────┘
           │  NATS outbound 연결 (에지가 중앙 NATS에 접속)
           │
...
릴리스 발행
  ...
  → edge-agent 수신
  → Local Harbor에서 image pull
  → kubectl apply CatalogPackage CRD
  → Update Operator reconcile
  → POST /agent/v1/deployment-result (결과 보고)
```
...
- **인가**: JWT `realm_roles` 클레임 기반 역할 체크 (Portal API 핸들러에서 처리)
  - `central-operator`: 릴리스 생성/발행, 전체 에지 승인, 원격 세션 생성
  - `edge-admin`: 자신의 에지에 대한 승인만 가능
